...
public class VideoController {
    ...
    public R publishVideo(@RequestBody @Validated Video video, HttpServletRequest request) {
        log.info("Received /video request, Authorization: {}", request.getHeader("Authorization"));
        Long userId = UserHolder.get();
        if (userId == null) {
            log.warn("UserHolder.get() returned null, token: {}", request.getHeader("Authorization"));
            return R.error().message("用户未登录或身份验证失败了QAQ");
        }
        video.setUserId(userId); // 在控制器中设置 user_id
        try {
            videoService.publishVideo(video);
            log.info("Video published for user: {}", userId);
            return R.ok().message("发布成功,请等待审核");
        } catch (Exception e) {
            log.error("Publish failed: {}", e.getMessage());
            return R.error().message("发布失败: " + e.getMessage());
        }
    }
